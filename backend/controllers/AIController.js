...
//   "controllerCode": "exports.controllername = async (req, res) => { ... }", // Use try-catch and return success, error properly
//   "testCases": [
//     {
//       "name": "Test name",
//       "input": {
...
// - endpoint should only be like /users/:id/profile if params if there
...
// {
//   "name": "Get All Tasks",
//   "description": "Retrieve all tasks for a specific user by user ID",
//   "method": "GET",
//   "endpoint": "/users/:id/tasks",
//   "controllerName": "getAllTasks",
//   "controllerCode": "exports.getAllTasks = async (req, res) => {
//                         try {
...
//                           return res.status(200).json({ success: true, data: tasks, message: 'Tasks retrieved successfully' });
//                         } catch (error) {
//                           return res.status(500).json({ success: false, message: 'An error occurred while retrieving tasks', error: error.message });
//                         }
//                       }"
//                       ,
//   "imports": [
...
// IMPORTANT RULES FOR MONGOOSE:
// - All models are Mongoose models, NOT SQL/Sequelize
// - Use Mongoose query methods: find(), findById(), findOne(), create(), findByIdAndUpdate(), findByIdAndDelete(), etc.
// - MongoDB uses _id as the primary key, but when referencing in schemas, use field names like userId, not user_id
...
// - For finding by ID, use findById(id) or find({ _id: id })
// - For relationships, use proper field naming (userId, not user_id) and populate when needed
// - Example queries:
...
// GENERAL RULES:
// - Format everything in valid JSON only (no markdown, no backticks)
// - controllerCode MUST be in format: exports.functionName = async (req, res) => { ... }
// - controllerName should be the exact function name from controllerCode (e.g., "getUserProfile", "createUser")
// - Include imports array with all required modules
// - For database models, use format: "../models/ModelName"
// - Available models: ${availableModels.join(', ')}
// - Use appropriate models based on the API functionality
// - Include proper error handling with try-catch blocks
// - Use appropriate HTTP status codes and error handling
// - Follow RESTful best practices
// - Use lowercase route naming convention
// - Consider the database entities provided for proper data modeling
// - Endpoint should be like /users/:id/profile if params are there
// - Only provide one test case for success scenario
// - Make the API production-ready with proper validation
// - Controller code should be perfectly generated with \\n for next line 
// - Controller should starts with exports.controllername = async (req, res) 
// - Always validate ObjectIds before using them in queries
// - Use proper Mongoose error handling for validation errors and cast errors
// - importstatememt const name = require("something"); where name should be relevant as per the controller code 
...
//         success: true,
//         api: fallbackApi,
//         message: 'API generated successfully (using fallback)',
