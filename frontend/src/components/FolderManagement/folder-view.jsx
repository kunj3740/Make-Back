...
const FolderView = ({ folders, setFolders, onFolderClick, projectId, userId, authToken }) => {
  ...
  const checkDuplicateFolderName = (name, excludeId = null) => {
    return folders.some(
      (folder) => folder.name.toLowerCase().trim() === name.toLowerCase().trim() && folder._id !== excludeId,
    )
  }
  ...
  const createFolder = async () => {
    ...
    if (checkDuplicateFolderName(newFolder.name.trim())) {
      setError(`A folder named "${newFolder.name.trim()}" already exists. Please choose a different name.`)
      return
    }
  ...
  const updateFolder = async (folderId, updatedData) => {
    ...
    if (updatedData.name && checkDuplicateFolderName(updatedData.name.trim(), folderId)) {
      setError(
        `A folder named "${updatedData.name.trim() || "this name"}" already exists. Please choose a different name.`,
      )
      return
    }
