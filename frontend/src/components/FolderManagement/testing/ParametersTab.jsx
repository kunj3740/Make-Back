...
"use client"
import { Plus, Trash2, Hash, Link } from "lucide-react"
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="text-xl font-bold text-white">Path Parameters</h3>
                <p className="text-slate-400 text-sm">Parameters extracted from the URL path (e.g., {"{id}"})</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            {pathParams.map((param, index) => (
              <div
                key={index}
                className="group p-4 bg-gradient-to-r from-purple-800/20 to-purple-700/20 rounded-xl border border-purple-600/30 hover:border-purple-500/50 transition-all duration-200 backdrop-blur-sm shadow-lg"
              >
                <div className="flex items-center space-x-4">
                  <div className="flex items-center">
                    <input
                      type="checkbox"
                      checked={param.enabled}
                      onChange={(e) => onUpdatePathParam(index, "enabled", e.target.checked)}
                      className="w-5 h-5 text-purple-500 bg-slate-800 border-slate-600 rounded focus:ring-purple-500/50 focus:ring-2"
                    />
                  </div>
                  <input
                    type="text"
                    value={param.key}
                    readOnly
                    className="flex-1 px-4 py-3 bg-slate-800/60 border border-slate-600/50 rounded-lg text-sm text-purple-300 focus:outline-none cursor-not-allowed opacity-75"
                    placeholder="Parameter name"
                  />
                  <input
                    type="text"
                    value={param.value}
                    onChange={(e) => onUpdatePathParam(index, "value", e.target.value)}
                    className="flex-1 px-4 py-3 bg-slate-800/80 border border-slate-600/50 rounded-lg text-sm text-slate-100 focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500/50 transition-all duration-200 placeholder-slate-500"
                    placeholder="Parameter value"
                  />
                  {onRemovePathParam && (
                    <button
                      onClick={() => onRemovePathParam(index)}
                      className="p-3 text-red-400 hover:bg-red-400/10 rounded-lg transition-all duration-200 opacity-0 group-hover:opacity-100 hover:shadow-lg"
                    >
                      <Trash2 className="h-4 w-4" />
                    </button>
