...
import React from 'react';
import { Play, Edit3, CheckCircle, X, Globe, ChevronDown } from 'lucide-react';
import { getMethodColor, HTTP_METHODS } from '../../../lib/utils';
...
const RequestBuilder = ({
  ...
  return (
    <div className="p-6 border-b border-slate-700/50 bg-gradient-to-br from-slate-800/50 to-slate-900/50">
      {/* Main Request Row */}
      <div className="flex items-stretch bg-white/5 rounded-lg border border-slate-600/30 overflow-hidden shadow-lg backdrop-blur-sm">
        {/* Method Dropdown */}
        <div className="relative">
          <select
            value={testRequest.method}
            onChange={(e) => onMethodChange(e.target.value)}
            className={`appearance-none px-4 py-4  font-bold text-sm border-0 focus:ring-0 focus:outline-none transition-all duration-200 cursor-pointer bg-slate-800 hover:bg-slate-700 ${
              testRequest.method === 'GET' ? 'text-emerald-400' :
              testRequest.method === 'POST' ? 'text-amber-400' :
              testRequest.method === 'PUT' ? 'text-blue-400' :
              testRequest.method === 'DELETE' ? 'text-red-400' :
              testRequest.method === 'PATCH' ? 'text-purple-400' :
              'text-slate-400'
            }`}
          ...
          <ChevronDown className={`absolute right-3 top-1/2 transform -translate-y-1/2 h-4 w-4 pointer-events-none ${
            testRequest.method === 'GET' ? 'text-emerald-500' :
            testRequest.method === 'POST' ? 'text-amber-500' :
            testRequest.method === 'PUT' ? 'text-blue-500' :
            testRequest.method === 'DELETE' ? 'text-red-500' :
            testRequest.method === 'PATCH' ? 'text-purple-500' :
            'text-slate-400'
          }`} />
        </div>

        {/* URL Input Container */}
        <div className="flex-1 flex border-l border-slate-600/30">
          {/* Complete URL Input */}
            ...
            {showBaseUrlEdit ? (
              <div className="flex w-full">
                <input
                  ...
                  value={tempBaseUrl}
                  onChange={(e) => onTempBaseUrlChange(e.target.value)}
                  className="flex-1 px-4 py-4 bg-slate-700/60 border-0 text-emerald-400 font-mono text-sm focus:outline-none focus:bg-slate-700/80 backdrop-blur-sm"
                ...
                <button
                  onClick={onBaseUrlUpdate}
                  className="px-3 py-3 bg-emerald-600 hover:bg-emerald-700 text-white transition-all duration-200"
                >
                  <CheckCircle className="h-4 w-4" />
                </button>
                <button
                  onClick={onBaseUrlCancel}
                  className="px-3 py-3 bg-slate-600 hover:bg-slate-700 text-slate-300 transition-all duration-200"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>
              ...
              <div className="flex-1 flex items-center px-4 py-3 bg-transparent">
                <button
                  onClick={() => onBaseUrlEdit(true)}
                  className="text-slate-400 hover:text-emerald-600 transition-colors font-mono text-sm flex items-center space-x-1 group"
                >
                  <span className='text-emerald-600 font-bold'>{`{{base_url}}`}</span>
                  <Edit3 className="h-3 w-3 opacity-0 group-hover:opacity-100 transition-all duration-200" />
                </button>
                <input
                  type="text"
                  value={testRequest.endpoint}
                  onChange={(e) => onEndpointChange(e.target.value)}
                  className="flex-1 bg-transparent border-0 font-mono text-sm text-slate-100 focus:outline-none placeholder-slate-500 ml-0"
                  placeholder="/api/v1/endpoint"
                />
              </div>
            )}
        ...
        <button
          onClick={onSendRequest}
          disabled={loading || !testRequest.endpoint}
          className="px-6 py-3 bg-gradient-to-r from-emerald-700 to-emerald-800 hover:from-emerald-700 hover:to-emerald-700 disabled:from-slate-600 disabled:to-slate-700 disabled:text-slate-400 text-white font-bold transition-all duration-200 flex items-center space-x-2 border-l border-emerald-600/30 disabled:border-slate-600/30 hover:shadow-lg disabled:shadow-none transform hover:scale-[1.02] disabled:hover:scale-100"
        >
          {loading ? (
            <>
      ...
      {/* URL Preview (Postman-style) */}
      <div className="mt-3 px-4 py-2 bg-slate-800/40 rounded-md border border-slate-700/30 backdrop-blur-sm">
        <div className="text-xs font-mono text-slate-400 flex items-center space-x-1">
          ...
          <span className="text-emerald-500">{baseUrl}</span>
          <span className="text-slate-200">{testRequest.endpoint}</span>
        </div>
      </div>
