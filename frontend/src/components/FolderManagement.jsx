...
const FolderManagement = ({
  ...
  const checkDuplicateApiName = (name, excludeId = null) => {
    if (!selectedFolder?.apis) return false
    return selectedFolder.apis.some(
      (api) => api.name.toLowerCase().trim() === name.toLowerCase().trim() && api._id !== excludeId,
    )
  }
  ...
  const handleGenerateAPI = async () => {
    ...
    if (checkDuplicateApiName(aiPrompt.name.trim())) {
      setAiError(
        `An API named "${aiPrompt.name.trim()}" already exists in this folder. Please choose a different name.`,
      )
      return
    }
    ...
    try {
      ...
      if (response.data.success) {
        const generatedAPI = response.data.api

        if (checkDuplicateApiName(generatedAPI.name)) {
          setAiError(
            `Generated API name "${generatedAPI.name}" already exists in this folder. Please try with a different name or description.`,
          )
          return
        }
      ...
      if (
        ...
        setAiError(`An API with this name already exists in the folder. Please choose a different name.`)
      } else {
  ...
  const handleApplyGeneratedAPI = async () => {
    ...
    if (checkDuplicateApiName(generatedAPI.name)) {
      setError(
        `An API named "${generatedAPI.name}" already exists in this folder. Please regenerate with a different name.`,
      )
      return
    }
    ...
    try {
      ...
      if (
        ...
        setError(`An API named "${generatedAPI.name}" already exists in this folder. Please choose a different name.`)
      } else {
  ...
  const handleApplyGeneratedUpdateAPI = async () => {
    if (!generatedUpdateAPI) return

    setError("")
    try {
      await axios.put(
        `${BACKEND_URL}/api/v1/folders/${selectedFolder._id}/api/${selectedApi._id}`,
        generatedUpdateAPI,
        {
          headers: { Authorization: `Bearer ${authToken}` },
        },
      ...
      setFolders((prev) =>
        prev.map((f) =>
          f._id === selectedFolder._id
            ? {
                ...f,
                apis: f.apis.map((api) => (api._id === selectedApi._id ? { ...api, ...generatedUpdateAPI } : api)),
              }
            : f,
        ),
      )
      ...
      setSelectedFolder((prev) => ({
        ...prev,
        apis: prev.apis.map((api) => (api._id === selectedApi._id ? { ...api, ...generatedUpdateAPI } : api)),
      }))
      ...
      if (
        ...
        setError(
          `An API named "${generatedUpdateAPI.name}" already exists in this folder. Please choose a different name.`,
        )
      } else {
  ...
  const updateApi = async (apiId, updatedData) => {
    ...
    if (updatedData.name && checkDuplicateApiName(updatedData.name.trim(), apiId)) {
      setError(
        `An API named "${updatedData.name.trim()}" already exists in this folder. Please choose a different name.`,
      )
      return
    }
    ...
    try {
      ...
      if (
        ...
        setError(
          `An API named "${updatedData.name?.trim() || "this name"}" already exists in this folder. Please choose a different name.`,
        )
      } else {
        setError("Failed to update API")
      }
