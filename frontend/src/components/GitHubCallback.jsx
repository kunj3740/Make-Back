  ...
  useEffect(() => {
    const handleCallback = async () => {
      try {
        ...
        // Check stored state for CSRF protection
        const storedState = localStorage.getItem("github_oauth_state") || 
                           sessionStorage.getItem("github_oauth_state")
        ...
        // Check if this is a project-specific auth (for GitHub push)
        const returnProjectId = sessionStorage.getItem("github_auth_return_project")
        ...
        // Store JWT token if it's a new login
        if (res.data.token) {
          localStorage.setItem("token", res.data.token)
        }
        
        // Redirect back to project if that's where they came from, with a flag
        if (returnProjectId) {
          sessionStorage.removeItem("github_auth_return_project")
          // Add a query param to tell the project page this is from auth
          navigate(`/projects/${returnProjectId}?from_auth=github&code=${code}&state=${state}`)
        } else {
          navigate("/projects")
        }
        ...
        // Check if there's a project to return to
        const returnProjectId = sessionStorage.getItem("github_auth_return_project")
        if (returnProjectId) {
          sessionStorage.removeItem("github_auth_return_project")
